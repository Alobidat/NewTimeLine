...
_LLM_PROVIDERS = [
    {
        # Ollama Cloud — offloads chat/script work off the local GPU (which is dedicated to the
        # ComfyUI render backend). OpenAI-compatible; key from env OLLAMA_API_KEY.
        "name": "ollama-cloud", "kind": "openai_compatible",
        "base_url": "https://ollama.com/v1", "model": "glm-5.2",
        "is_local": False, "api_key_env": "OLLAMA_API_KEY",
    },
    {
...
SPECS: list[ConfigSpec] = [
    ...
    ConfigSpec(key="llm.routing", type="json", scope="llm",
               default={"primary": "ollama-cloud", "fallback": "ollama"},
    ...
    _i("agents.news_video.scenes", 5, "Scenes per clip",
       scope="agent:comfyui", component_id="agent:bots.news_video", minimum=2, maximum=8,
       help="How many image+caption beats the explainer is built from (also # narration lines)."),
    _i("agents.news_video.image_steps", 30, "SDXL steps / scene",
       scope="agent:comfyui", component_id="agent:bots.news_video", minimum=10, maximum=60),
    _i("agents.news_video.image_width", 768, "Scene image width",
       scope="agent:comfyui", component_id="agent:bots.news_video", minimum=512, maximum=1344,
       help="Scene image size — 768x1344 is SDXL's 9:16 bucket (phone full-bleed, no crop)."),
    _i("agents.news_video.image_height", 1344, "Scene image height",
       scope="agent:comfyui", component_id="agent:bots.news_video", minimum=512, maximum=1536),
    _b("agents.news_video.tts_enabled", True, "Voiceover (Piper TTS)",
       scope="agent:comfyui", component_id="agent:bots.news_video",
       help="Narrate the summary over the visuals; off = captions only (silent)."),
    ConfigSpec(key="agents.news_video.voice_model", type="string", scope="agent:comfyui",
               default="/opt/piper/en_US-amy-medium.onnx", label="Piper voice model path",
               component_id="agent:bots.news_video",
               help="Path to the Piper .onnx voice in the agents image."),
