/// The interactive timeline panel: scrub (drag), zoom (pinch / mouse wheel), tap an event
/// to open detail + sub-timeline. Hosted by AppShell beneath the map; shares its window.
library;
...
class _TimelinePanelState extends State<TimelinePanel> {
  ...
  void _onTapUp(TapUpDetails d) {
    ...
    if (data == null || data.isBuckets) {
      return; // can't tap individual events when bucketed (zoomed out)
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        _PresetBar(onSelect: _c.setRange),
      ],
    );
  }
}
...
class _PresetBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SingleChildScrollView(
    ...
    child: Row(
      children: [
        for (final p in _TimelinePanelState._presets)
          Padding(
